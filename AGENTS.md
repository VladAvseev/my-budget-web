...
- **Работа с API:** только через `apiGet`/`apiPost` из `@/shared/api` (общий axios-клиент с таймаутом). Поддерживается передача `signal` для отмены запросов.
...
Для работы с базой данных в проект будет использовать supabase, организовывать запросы к supabase необходимо таким образом, чтобы в будущем была возможность легко перейти с supabase на собственный backend c API и базой данных. Так же данные запросы должны быть совместимы с Tanstack Query
